export const sidebarNavigations = [
    { icon: 'pi pi-home', title: 'Overview', href: '/main/dashboard' },
    { icon: 'pi pi-user', title: 'User Management', href: '/main/management/user' },
    { icon: 'pi pi-truck', title: 'Device Management', href: '/main/management/device' },
    { icon: 'pi pi-map', title: 'Tracking', href: '/main/management/tracking'},
    { icon: 'pi pi-wave-pulse', title: 'Reports', href: '/main/reports/all' }
];

export const moreSidebarNavigations = [
    { icon: 'pi pi-cog', title: 'Settings' }
];